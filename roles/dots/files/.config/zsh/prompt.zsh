...
zstyle ':vcs_info:*' stagedstr "%F{red}●%f" # default 'S'
zstyle ':vcs_info:*' unstagedstr "%F{green}●%f" # default 'U'
...
function () {
  ...
  # export PS1="%F{blue}%B%1~%b%F{yellow}%B%(1j.*.)%b%f %B%(?.%{$fg[red]%}.%{$fg[green]%})%(!.#.${SUFFIX})%{$reset_color%} "
